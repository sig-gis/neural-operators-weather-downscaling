...
def get_model(config, mean=None, std=None, upsampling_factor=8):
    ...
    * Adds mean, std and upsampling_factor to the config["arch"]'s (model's) config
    ...
    * some keyword arguments of the models aren't provided by the config
    ...
    config_arch["mean"] = mean
    config_arch["std"] = std
    config_arch["upscale"] = upsampling_factor

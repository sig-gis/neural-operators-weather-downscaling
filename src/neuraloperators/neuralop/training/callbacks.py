...
import wandb
from src.neuraloperators.neuralop.training.patching import MultigridPatching2D
...
class BasicLoggerCallback(Callback):
    ...
    def on_train_start(self, **kwargs):
        ...
        valid_loader = self.state_dict['valid_loader']
        ...
        if verbose:
            ...
            # print(f'Testing on {[(len(loader.dataset), loader.dataset[0]["x"].shape) for loader in test_loaders.values()]} samples'
...
class MatplotlibWandBLoggerCallback(BasicLoggerCallback):
    '''
    Adds matplotlib functionality to the WandB logger (nothing added as of now)
    y: the ground truth value
    out: the NN prediction of y
    '''
    ...
    def on_val_epoch_end(self, errors, sample, out, epoch):
        y=sample['y']

        for loss_name, loss_value in errors.items():
            # loss_value = de_normalize_images(loss_value)
            if isinstance(loss_value, float):
                self.state_dict['msg'] += f', {loss_name}={loss_value:.4f}'
            else:
                loss_value = {i:e.item() for (i, e) in enumerate(loss_value)}
...
class CheckpointCallback(Callback):
    ...
    def on_train_start(self, *args, **kwargs):
        ...
        if self.save_best:
            # print(self.state_dict['eval_losses'].keys())
        ...
        if self.resume_from_dir:
            ...
            if 'best_model_state_dict' in saved_modules:
                if hasattr(self.state_dict['model'], 'load_checkpoint'):
                    self.state_dict['model'].load_checkpoint(save_folder = self.resume_from_dir, save_name = 'best_model_state_dict')
                else: 
                    self.state_dict['model'].load_state_dict(torch.load(self.resume_from_dir / 'best_model_state_dict.pt'))
                if verbose:
                    print(f"Loading model state from best_model_state_dict.pt")
            else:
                if hasattr(self.state_dict['model'], 'load_checkpoint'):
                    self.state_dict['model'].load_checkpoint(save_folder = self.resume_from_dir, save_name = 'model_state_dict')
                else: 
                    self.state_dict['model'].load_state_dict(torch.load(self.resume_from_dir / 'model_state_dict.pt'))
                if verbose:

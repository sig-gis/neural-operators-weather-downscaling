...
import src.neuraloperators.neuralop.mpu.comm as comm
from src.neuraloperators.neuralop.losses import LpLoss
...
class Trainer:
    ...
    def train(self, train_loader, valid_loader,
        ...
        if self.callbacks:
            self.callbacks.on_train_start(train_loader=train_loader, valid_loader = valid_loader,
        ...
        for epoch in range(self.n_epochs):
            ...
            for idx, sample in enumerate(train_loader):
                ...
                if self.data_processor is not None:
                    ...
                    sample = {"x":sample[0].to(self.device), "y":sample[1].to(self.device)}
                    # {k:v.to(self.device) for x,y in sample if torch.is_tensor(v)}

                if epoch==0 and idx==0:
                    print("train first batch shape: ", sample.keys(), sample["x"].shape, sample["y"].shape)
            ...
            if epoch % self.log_test_interval == 0: 
                ...
                _ = self.evaluate(eval_losses, valid_loader, log_prefix="valid_loss")
    ...
    def evaluate(self, loss_dict, data_loader,
        ...
        if self.callbacks:
            self.callbacks.on_val_epoch_end(errors=errors, sample=sample, out=out, epoch=self.current_epoch)
